...
import { RegisterRequestSchema } from '@wildtails/contracts';

/**
 * POST /api/auth/register
 * Proxies the register request to NestJS, extracts the refresh token into
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const response = await nestjsFetch('/auth/register', {
    method: 'POST',
  ...
  return NextResponse.json({ success: true, data: browserData }, { status: 201 });
}
